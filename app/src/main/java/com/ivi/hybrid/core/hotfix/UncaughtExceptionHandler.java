...
final class UncaughtExceptionHandler implements Thread.UncaughtExceptionHandler {
    private static final String TAG = "Tinker.UncaughtExceptionHandler";
    ...
    /**
     * Such as Xposed, if it try to load some class before we load from patch files.With dalvik,
     * it will crash with "Class ref in pre-verified class resolved to unexpected implementation".
    ...
    private void tinkerPreVerifiedCrashHandler(Throwable ex) {
        ApplicationLike applicationLike = TinkerAppLike.getInstance();
