...
public class NotifyUtil {
    ...
    public NotifyUtil(Context context, int ID) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            nm.createNotificationChannelGroup(new NotificationChannelGroup("a", "a"));
            NotificationChannel channel = new NotificationChannel("2",
                    "Channel1", NotificationManager.IMPORTANCE_DEFAULT);
            nm.createNotificationChannel(channel);
            cBuilder = new NotificationCompat.Builder(mContext, "2");
        } else {
    ...
    private void setBuilder(PendingIntent pendingIntent, int smallIcon, String ticker, boolean sound, boolean vibrate, boolean lights) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            NotificationChannel channel = new NotificationChannel("2",
                    "Channel1", NotificationManager.IMPORTANCE_DEFAULT);
            nm.createNotificationChannel(channel);
            nBuilder = new Notification.Builder(mContext, "2");
        } else {
...
//                requestCode, intent, FLAG);
    ...
    public void notify_progress(PendingIntent pendingIntent, int smallIcon,
                                String ticker, String title, String content, boolean sound, boolean vibrate, boolean lights, int progress, int max) {

        setCompatBuilder(pendingIntent, smallIcon, ticker, title, content, sound, vibrate, lights);
        cBuilder.setProgress(max, progress, false);
        if (progress >= max) {
